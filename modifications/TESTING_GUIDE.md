...
16. [Unit Tests for Rate Limiting (`apps/web`)](#16-unit-tests-for-rate-limiting-appsweb)
17. [Unit Tests for Warm Browser Pool (`apps/agent`)](#17-unit-tests-for-warm-browser-pool-appsagent)
18. [Integration Tests for Per-Provider Queue System](#18-integration-tests-for-per-provider-queue-system)
19. [Integration Tests for tRPC Rate Limiting Middleware](#19-integration-tests-for-trpc-rate-limiting-middleware)
...
**Rate limiting is silent until it isn't.** The fixed-window Lua script in `apps/web/src/lib/rate-limit.ts` runs atomically in Redis. If the TTL-on-first-increment logic is wrong, every window after the first is infinite — you never get throttled. Unit tests that replay the exact counter sequence are the only reliable way to catch this.

**Per-provider workers mean 5× the failure modes.** Each provider (`openai`, `anthropic`, `perplexity`, `google`, `google-ai-overview`) now has its own BullMQ queue and worker. A bug in `submitAgentJobGroup` — like only submitting to the first provider — would silently drop 4 of 5 jobs. Integration tests that assert "exactly N jobs were enqueued across N queues" are the only way to catch this.
...
The proxy pool is split into two files with pure-function APIs — `scoring.ts` (the math) and `pool.ts` (the stateful pool). This separation makes unit testing straightforward: `scoring.ts` functions are completely pure (no side effects), and `pool.ts` state is module-level and can be reset between tests.

**Important:** The proxy records live in an in-memory `Map` inside `pool.ts`. There is no class to instantiate — you call the exported functions directly and reset module state between tests using `vi.resetModules()`.

```typescript
// apps/agent/src/__tests__/proxyScoring.test.ts
// Tests the PURE scoring functions in scoring.ts — no state, no mocks needed

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { getProxyScore, getCooldownMs, type ProxyRecord } from '../lib/browser/proxy/scoring';
...
describe('getProxyScore', () => {
  ...
  it('returns 0.5 (neutral) for a proxy with no history', () => {
    ...
    expect(getProxyScore(record)).toBe(0.5);
  });

  it('returns 1.0 after a single recent success', () => {
    const record: ProxyRecord = {
      proxy: '1.2.3.4:9000',
      events: [{ timestamp: Date.now(), success: true }],
      cooldownUntil: 0,
      consecutiveFailures: 0,
    };
    expect(getProxyScore(record)).toBe(1.0);
  });

  it('returns 0.0 after a single recent failure', () => {
    const record: ProxyRecord = {
      proxy: '1.2.3.4:9000',
      events: [{ timestamp: Date.now(), success: false, failureType: 'bot_detection' }],
      cooldownUntil: 0,
      consecutiveFailures: 1,
    };
    expect(getProxyScore(record)).toBe(0.0);
  });

  it('decays old failures toward 0.5 over time (10-minute half-life)', () => {
    // Record a failure 30 minutes ago (3 half-lives)
    const thirtyMinutesAgo = Date.now() - 30 * 60 * 1000;
    const record: ProxyRecord = {
      proxy: '1.2.3.4:9000',
      events: [{ timestamp: thirtyMinutesAgo, success: false, failureType: 'bot_detection' }],
      cooldownUntil: 0,
      consecutiveFailures: 0,
    };
    const score = getProxyScore(record);
    // After 3 half-lives the weight is 0.5^3 = 0.125, so score approaches 0 but isn't 0
    // A recent failure would give 0.0; an old failure gives something closer to 0.5
    expect(score).toBeGreaterThan(0.0);
    expect(score).toBeLessThan(0.5);
  });

  it('mixed history: 2 successes + 1 failure scores above 0.5', () => {
    const now = Date.now();
    const record: ProxyRecord = {
      proxy: '1.2.3.4:9000',
      events: [
        { timestamp: now, success: true },
        { timestamp: now - 1000, success: true },
        { timestamp: now - 2000, success: false, failureType: 'connection_error' },
      ],
      cooldownUntil: 0,
      consecutiveFailures: 0,
    };
    expect(getProxyScore(record)).toBeGreaterThan(0.5);
  });
});

describe('getCooldownMs', () => {

  it('bot_detection: 60s base, doubles per failure, caps at 15 minutes', () => {
    expect(getCooldownMs('bot_detection', 1)).toBe(60_000);
    expect(getCooldownMs('bot_detection', 2)).toBe(120_000);
    expect(getCooldownMs('bot_detection', 3)).toBe(240_000);
    // Cap at 15 minutes:
    expect(getCooldownMs('bot_detection', 10)).toBe(15 * 60 * 1000);
  });

  it('rate_limited: 30s base, doubles per failure, caps at 5 minutes', () => {
    expect(getCooldownMs('rate_limited', 1)).toBe(30_000);
    expect(getCooldownMs('rate_limited', 2)).toBe(60_000);
    expect(getCooldownMs('rate_limited', 10)).toBe(5 * 60 * 1000);
  });

  it('connection_error: 10s base, doubles per failure, caps at 10 minutes', () => {
    expect(getCooldownMs('connection_error', 1)).toBe(10_000);
    expect(getCooldownMs('connection_error', 2)).toBe(20_000);
    expect(getCooldownMs('connection_error', 10)).toBe(10 * 60 * 1000);
  });

  it('logged_out: 30s * failures, caps at 5 minutes', () => {
    expect(getCooldownMs('logged_out', 1)).toBe(30_000);
    expect(getCooldownMs('logged_out', 3)).toBe(90_000);
    expect(getCooldownMs('logged_out', 20)).toBe(5 * 60 * 1000);
  });

  it('unknown failure type: 15s * failures, caps at 5 minutes', () => {
    expect(getCooldownMs(undefined, 1)).toBe(15_000);
    expect(getCooldownMs(undefined, 10)).toBe(5 * 60 * 1000);
  });
});
```
...
// Tests the stateful pool module — needs module reset between tests

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';

// Re-import module fresh each test to reset in-memory Map state
async function freshPool() {
  vi.resetModules();
  return import('../lib/browser/proxy/pool');
}

describe('proxy pool (pool.ts)', () => {

  beforeEach(() => { vi.useFakeTimers(); });
  afterEach(() => { vi.useRealTimers(); });

  it('returns null when no proxies are loaded', async () => {
    const { getNextProxy } = await freshPool();
    expect(getNextProxy()).toBeNull();
  });

  it('skips proxies in cooldown, returns the non-cooldown proxy', async () => {
    const { getNextProxy, recordProxyResult, loadProxies } = await freshPool();

    loadProxies(['1.1.1.1:9000', '2.2.2.2:9000']);
    // Put first proxy in cooldown via a bot_detection failure:
    recordProxyResult('1.1.1.1:9000', false, 'bot_detection');

    // Should always return the non-cooldown proxy:
    const result = getNextProxy();
  ...
  it('returns null when all proxies are in cooldown', async () => {
    ...
    loadProxies(['1.1.1.1:9000']);
    recordProxyResult('1.1.1.1:9000', false, 'bot_detection');

    expect(getNextProxy()).toBeNull();
  });

  it('proxy becomes available again after cooldown expires', async () => {
    const { getNextProxy, recordProxyResult, loadProxies } = await freshPool();

    loadProxies(['1.1.1.1:9000']);
    recordProxyResult('1.1.1.1:9000', false, 'rate_limited'); // 30s cooldown

    expect(getNextProxy()).toBeNull(); // In cooldown

    vi.advanceTimersByTime(31_000); // Advance past cooldown
    expect(getNextProxy()).toContain('1.1.1.1');
  });

  it('records success: resets consecutiveFailures and cooldownUntil', async () => {
    const { getNextProxy, recordProxyResult, loadProxies } = await freshPool();

    loadProxies(['1.1.1.1:9000']);
    recordProxyResult('1.1.1.1:9000', false, 'bot_detection'); // Into cooldown
    expect(getNextProxy()).toBeNull();

    recordProxyResult('1.1.1.1:9000', true); // Success resets cooldown
    expect(getNextProxy()).toContain('1.1.1.1');
  });

  it('ring-buffer caps events at MAX_EVENTS (20)', async () => {
    // Import scoring to check MAX_EVENTS:
    vi.resetModules();
    const { MAX_EVENTS } = await import('../lib/browser/proxy/scoring');
    const { recordProxyResult, loadProxies } = await import('../lib/browser/proxy/pool');

    loadProxies(['1.1.1.1:9000']);
    // Record 25 events:
    for (let i = 0; i < 25; i++) {
      recordProxyResult('1.1.1.1:9000', i % 2 === 0);
    }

    // Proxy records are internal — verify indirectly via getNextProxy behavior
    // (the pool should still function, not throw or accumulate unbounded memory)
