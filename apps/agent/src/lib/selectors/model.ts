...
export function buildSystemPrompt(stage: SelectorStage): string {
	...
	if (stage === "response") {
		return `${shared} Task: identify response, generationIndicator, and sourcesButton. response: the outermost container of the LATEST model answer body only — not a child paragraph, code block, whole-page wrapper, research/status header, tool-use summary, reasoning/thinking panel, citation tray, or metadata badge. Must contain the full final answer text, must be absent for prior turns, and must persist after streaming ends. If a short framing/preamble sentence introduces the answer and is followed by the real structured answer, choose the container for the real answer body, not a wrapper whose main value is the preamble. Reject any candidate that contains editable elements, user input, multiple historical answers, or short status/label rows that describe the answer instead of being the answer itself. generationIndicator: a small UI element (stop button, spinner, live-region) that is ONLY visible while the answer is streaming and disappears when complete. Must not contain the answer body. Return [] if no such element is visible. sourcesButton: the SINGLE clickable control that reveals or switches to a dedicated sources/citations list for this latest answer. This includes: a standard button near the response, a tab ([role='tab'] inside a [role='tablist']) that switches the view to show a list of sources, a toggle that expands a citations panel, or any other interactive element that when activated shows a dedicated list of external references. Return [] ONLY when: (a) there is truly no such control anywhere near or below the response, AND either the citations are purely inline with no separate panel OR the sources panel is already permanently visible on the page without any click being required to show it. Do NOT return [] just because citations appear inline — if any dedicated sources control exists near the response, always return it.`;
	}

	return `${shared} Task: identify sourcePanel and sourceItem. The sources UI is already visible — either revealed after a button/tab click, or permanently displayed as a side-column or sidebar without any user action required. sourcePanel: return a selector for EVERY distinct container that holds sources for the latest answer, whether that container was opened by a click or was always present on the page (e.g. a persistent right-side sources column). GROUPING RULE: when multiple sibling lists (ul, ol, div) exist inside a single parent container, return the PARENT container as sourcePanel — not the individual sibling lists. Only return individual list elements as separate sourcePanel entries when they are in genuinely separate UI regions. The goal is one sourcePanel selector per distinct UI region, where each selector encompasses all source items within that region. Some providers render citations in multiple separate locations; include a selector for each distinct region. Do not include the full document, page root, or top-level layout wrappers. sourceItem: the single most generic selector that matches all individual source cards, rows, anchors, or citation links across ALL panels — it must work inside every sourcePanel container. If items differ by panel, prefer the selector that matches the most items.`;
}
