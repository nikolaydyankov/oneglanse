...
AGENT_AUTH_UPLOAD_TOKEN=your-secret-token   # same token as on the VPS
```
...
`pnpm auth` opens the provider sign-in browser, captures sessions, then asks whether to upload to the VPS — answer `y`. Sessions are transferred immediately.
